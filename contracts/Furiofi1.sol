...
/// @notice The contract keeps track of the liquidity pool balances, of the Furiofi staking pool lp tokens and the GHNY staking pool furFiToken rewards of a furiofi strategy investor using EIP-1973
...
abstract contract FuriofiStrategy is Initializable, BaseConfig {
    ...
    function furiofiStrategyWithdraw(uint256 amount) internal {
        require(amount > 0, "TZ");
        require(amount <= getFuriofiStrategyBalance(), "SD");
    ...
    /// @dev The furFiToken and lp rewards are requested from the GHNY staking pool for the whole contract
    ...
    /// @notice Claims the staked furFiToken for an investor. The investors honnies are first unstaked from the GHNY staking pool and then transfered to the investor.
    ...
    function getFuriofiStrategyBalance() public view returns (uint256) {
        return participantData[msg.sender].amount;
    }

    /// @notice Gets the current staked furFiToken for a furiofi strategy investor
    /// @dev Gets the current furFiToken balance from the GHNY staking pool to calculate the current furFiToken round mask. This is then used to calculate the total pending furFiToken for the investor
    /// @return The current furFiToken balance for a furiofi investor
    function getFuriofiStrategyStakedFurFi() public view returns (uint256) {
        if (
        ...
        return
            participantData[msg.sender].pendingFurFi +
            ((currentFurFiRoundMask - participantData[msg.sender].furFiMask) *
                participantData[msg.sender].amount) /
            DECIMAL_OFFSET;
    }
    ...
    function getFuriofiStrategyLpRewards() external view returns (uint256) {
        if (
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function withdrawAll(
        ...
        _stakeRewards();
        uint256 currentDeposits = getFuriofiStrategyBalance();
