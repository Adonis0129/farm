    ...
    PausableUpgradeable
{
    ...
    /// @notice The current referral rewards for a referralGiver dependent on the pool
    /// @dev Calculated using the referral rewards plus the difference from the current round mask
    /// @param _poolAddresses The addresses of the pool for which the rewards should be calculated
    /// @param _referralGiver The referral giver address
    /// @return the current rewards for the referral giver
    function getAllReferralRewards(address[] memory _poolAddresses, address _referralGiver)
    public
    view
    returns(uint256)
    {
        ...
        for (uint256 i = 0; i < _poolAddresses.length; i++) {
            uint256 _amount = getReferralRewards(_poolAddresses[i], _referralGiver);
    ...
    /// @param _referralGiver The referral giver address
    /// @return The total earned amount
    function getAllTotalEarnedAmount(address[] memory _poolAddresses, address _referralGiver)
    external
    view
    returns(uint256)
    {
        ...
        for (uint256 i = 0; i < _poolAddresses.length; i++) {
            uint256 _amount = getTotalEarnedAmount(_poolAddresses[i], _referralGiver);
            _totalEarned += _amount;
        }
        return _totalEarned;
    }
