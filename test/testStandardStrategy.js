...
var deployedAddress = {
  ...
  sDStrategyFurioFinance: "0x670E70BAE79d3cF3006B00D4A4E9B3df674813d0",
};
...
describe("Contract deployment and setting for farming", () => {
  ...
  it("(usdc-busd)SDStrategyFurioFinance contract deployment and set ", async () => {
    const SDStrategyFurioFinance = await ethers.getContractFactory("SDStrategyFurioFinance");
    if (!isOnchain) {
      //for hardhat test
      ...
      await tx.wait();
      var tx = await sDStrategyFurioFinance.grantRole(keccak256("PAUSER_ROLE"), owner.address);
      await tx.wait();
      //set restakeThreshold
      var tx = await sDStrategyFurioFinance.updateRestakeThreshold("0");
      await tx.wait();
      ...
      sDStrategyFurioFinance = SDStrategyFurioFinance.attach(deployedAddress.sDStrategyFurioFinance);
    }
...
/////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////       test  StandardStrategy contract            /////////////
/////////////////////////////////////////////////////////////////////////////////////////////

describe("test", () => {
  it("user1 deposit with 0.02 BNB and 5% slippage to (usdc-busd)SDStrategyFurioFinance ", async () => {
    if (!isOnchain) {
      var currentTimeStamp = (await ethers.provider.getBlock("latest")).timestamp;
      // console.log("current timestamp", currentTimeStamp);
  ...
  it("user2 deposit with 0.01 BNB and 5% slippage to (usdc-busd)SDStrategyFurioFinance ", async () => {
    if (!isOnchain) {
      var currentTimeStamp = (await ethers.provider.getBlock("latest")).timestamp;
  ...
  it("user1 deposit with 100 DAI and 10% slippage to (usdc-busd)SDStrategyFurioFinance ", async () => {
    if (!isOnchain) {
      ...
      await tx.wait();
      var tx = await fakeDAI.connect(user1).approve(sDStrategyFurioFinance.address, toBigNum("100", 18));
      await tx.wait();
      var currentTimeStamp = (await ethers.provider.getBlock("latest")).timestamp;
