...
var deployedAddress = {
  ...
  sCStrategyFurioFinance: "0x329707d220191D32CDe1b005B46C5c342b9e1D68",
};
...
describe("Contract deployment and setting for farming", () => {
  ...
  it("(usdc-busd)SCStrategyFurioFinance contract deployment and set ", async () => {
    const SCStrategyFurioFinance = await ethers.getContractFactory("SCStrategyFurioFinance");
    if (!isOnchain) {
      // for hardhat test
      ...
      await tx.wait();
      //Referral
      var tx = await referral.grantRole(keccak256("REWARDER_ROLE"), sCStrategyFurioFinance.address);
      await tx.wait();
...
/////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////          test  StableCoinStrategy contract                 //////////
/////////////////////////////////////////////////////////////////////////////////////////////

describe("test", () => {
  it("user1 deposit with 0.02 BNB and 5% slippage to (usdc-busd)SCStrategyFurioFinance ", async () => {
    if (!isOnchain) {
      var currentTimeStamp = (await ethers.provider.getBlock("latest")).timestamp;
      // console.log("current timestamp", currentTimeStamp);
  ...
  it("user2 deposit with 0.01 BNB and 5% slippage to (usdc-busd)SCStrategyFurioFinance ", async () => {
    if (!isOnchain) {
      var currentTimeStamp = (await ethers.provider.getBlock("latest")).timestamp;
  ...
  it("user1 deposit with 100 DAI and 10% slippage to (usdc-busd)SCStrategyFurioFinance ", async () => {
    if (!isOnchain) {
      var tx = await fakeDAI.transfer(user1.address, toBigNum("100", 18));
      await tx.wait();
      var tx = await fakeDAI.connect(user1).approve(sCStrategyFurioFinance.address, toBigNum("100", 18));
      await tx.wait();
      var currentTimeStamp = (await ethers.provider.getBlock("latest")).timestamp;
  ...
  it("user3 deposit with 30 USDC and 5% slippage to (usdc-busd)SCStrategyFurioFinance ", async () => {
    if (!isOnchain) {
      var tx = await fakeUSDC.transfer(user3.address, toBigNum("30", 18));
      await tx.wait();
      var tx = await fakeUSDC.connect(user3).approve(sCStrategyFurioFinance.address, toBigNum("30", 18));
      await tx.wait();
      var currentTimeStamp = (await ethers.provider.getBlock("latest")).timestamp;
  ...
  it("user1 withdraw All tokens  ", async () => {
    if (!isOnchain) {
      var tx = await token.connect(user1).approve(sCStrategyFurioFinance.address, token.balanceOf(user1.address));
      await tx.wait();

      var currentTimeStamp = (await ethers.provider.getBlock("latest")).timestamp;
      var tx = await sCStrategyFurioFinance.connect(user1).withdrawAll(
          [wBNB.address, wBNB.address],
          [fakeUSDC.address, fakeBUSD.address],
